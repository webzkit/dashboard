...
export function useLoginFormRules() {
    ...
    const getUsernameFormRule = computed(() => createRule(t("sys.login.validate.username")));
...
const createMinMaxRule = (message: string, minMaxMessage: string, min: number = 6, max: number = 18) => {
    return [
