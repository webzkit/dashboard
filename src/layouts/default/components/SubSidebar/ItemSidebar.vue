<template>
  <div class="sidebar-item">
    <el-sub-menu v-if="item.path === undefined" :title="t(item.meta.title)" :index="JSON.stringify(item)">
      <template #title>
      ...
      <template v-for="route in item.children">
        <ItemSidebar v-if="route.meta.sidebar !== false" :key="route.path" :item="route" />
      </template>
    </el-sub-menu>
    ...
    <router-link v-else-if="!hasChildren" v-slot="{ href, navigate, isActive, isExactActive }" custom
      ...
      <a :href="isExternalLink(resolveRoutePath(basePath, item.path)) ? resolveRoutePath(basePath, item.path) : href"
        :class="[isActive && 'router-link-active', isExactActive && 'router-link-exact-active']"
        :target="isExternalLink(resolveRoutePath(basePath, item.path)) ? '_blank' : '_self'" @click="navigate">
        <el-menu-item :title="t(item.meta.title)" :index="resolveRoutePath(basePath, item.path)">
          <Icon v-if="item.meta.icon" :icon="item.meta.icon" class="title-icon" />
          <span class="title">{{ t(item.meta.title) }}</span>
        </el-menu-item>
      </a>
    ...
    <el-sub-menu v-else :title="t(item.meta.title)" :index="resolveRoutePath(basePath, item.path)">
      <template #title>
        <Icon v-if="item.meta.icon" :icon="item.meta.icon" class="title-icon" />
        <span class="title">{{ t(item.meta.title) }}</span>
      </template>
