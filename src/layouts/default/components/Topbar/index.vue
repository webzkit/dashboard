<template>
  <div :class="[{ 'fixed': getFixed, 'shadow': scrollTop }, 'topbar-container']" data-fixed-calc-width>
    <div class="left-box">
      <div v-if="getEnableSidebarCollapse" :class="[{ 'is-collapse': getSubMenuCollaspe }, 'sidebar-collapse']"
      ...
      <el-breadcrumb v-if="isShowBreadcrumb">
        <transition-group name="breadcrumb">
          <el-breadcrumb-item v-for="(item, index) in breadcrumbes" :key="item.path"
...
const { getFixed, getEnableSidebarCollapse, getEnableBreadcrumb } = useHeaderSetting();
...
const isShowBreadcrumb = computed(() => {
  return unref(getEnableBreadcrumb) && unref(getDisplayMode) === DisplayModeEnum.PC && unref(getRouteBaseOn) !== RouteBaseOnEnum.FILE_SYSTEM;
});
...
const breadcrumbes = computed(() => {
  ...
  if (route.meta.breadcrumbNeste) {
    // @ts-ignore
    results.push(...route.meta.breadcrumbNeste.filter((item: any) => get(item, 'hide', false) === false));
  }
