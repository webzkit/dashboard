...
import { path as pathBrowserify } from "path-browserify";
...
const getDeepestPath = function (routes: any, rootPath: string = ''): string {
  ...
  if (routes.children.some((item: any) => {
    return item.meta.sidebar != false;
  })) {
    for (let i = 0; i < routes.children.length; i++) {
      if (routes.children[i].meta.sidebar != false) {
        return getDeepestPath(routes.children[i], resolveRoutePath(rootPath, routes.path))
      }
    }
  }

  return getDeepestPath(routes.children[0], resolveRoutePath(rootPath, routes.path))
}
...
export const useMenuStore = defineStore({
  ...
  getters: {
    ...
    getSidebarMenus(): string[] {
      return this.getMenus.length > 0 ? this.getMenus[this.actived].children : [];
    },

    // The path of the first navigation in the secondary navigation
    sidebarMenuFirstDeepestPath() {
      return this.getMenus.length > 0 ? getDeepestPath(this.getSidebarMenus[0]) : '/';
    },
    ...
    defaultOpenedPaths() {
      ...
      routeSetting.routeStore.getRoutes.map((item: any) => {
        ...
        item.children && item.children.map((child: any) => {
          child.meta.defaultOpened && defaultOpenedpaths.push(pathBrowserify.resolve(item.path, child.path))
        })
      })
